...
CREATE TABLE IF NOT EXISTS `caso2`.`sales` (
  ...
  `commission_id` INT NOT NULL DEFAULT 1,
  PRIMARY KEY (`sale_id`),
  INDEX `fk_sale_buyer1_idx` (`client_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `caso2`.`purchase_orders` (
  ...
  INDEX `fk_purchase_orders_work_schedule_logs1_idx` (`work_schedule_log_id` ASC) VISIBLE,
  CONSTRAINT `fk_purchase_orders_clients1`
    FOREIGN KEY (`client_id`)
    REFERENCES `caso2`.`clients` (`client_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_purchase_orders_work_schedule_logs1`
    FOREIGN KEY (`work_schedule_log_id`)
    REFERENCES `caso2`.`work_schedule_logs` (`work_schedule_log_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
